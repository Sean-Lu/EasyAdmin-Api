...
- 支持通过自然语言查询当前用户有权访问的笔记、待办、日报、周报、月报、通知和菜单
- 回答可关联实际引用的数据来源，方便查看和核对相关业务内容
- 支持生成笔记、待办、日报、周报和月报草稿，经用户确认后通过原有业务服务保存
- 支持多轮会话和历史记录，满足连续提问和上下文交流需求
- 超级管理员可配置 AI 模型服务、连接参数和生成参数，并查看租户 AI 用量
- 支持按租户启用 AI 功能、设置每日请求上限，实现租户级能力和用量管理
...
- [x] **AI 模型配置**：配置 AI 模型服务、连接参数和生成参数
- [x] **AI 用量记录**：查看各租户和用户的 AI 请求及令牌用量
...
- [x] **我的收藏**：个人收藏管理，支持收藏菜单、文件、笔记和文件/笔记分享链接
...
- [x] **AI 助手**：支持智能问答、多轮对话、来源查看和业务草稿生成

#### 🔗 外部链接

- [x] **Gitee 仓库**：新标签页打开 `Gitee` 仓库页面
- [x] **GitHub 仓库**：新标签页打开 `GitHub` 仓库页面
- [x] **百度搜索**：内嵌打开百度搜索页面
